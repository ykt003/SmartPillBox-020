...
public class MedDescriptionAddActivity extends Activity implements OnClickListener, OnLongClickListener{
	...
	public static MedDescriptionAddActivity instance;
	...
	public void onClick(View v) {
		...
		if (v == back) {
			...
			MedDescriptionAddActivity.this.finish();
		}
		//添加界面的删除
		if (v == delete) {
			MedDescriptionDeleteDialogActivity.showDialog_Layout(this, "s",gg);
		}
		...
		if (v == clock_on) {
			...
			if (flag == false) {
				...
				if(repeat == 0){
					...
					Toast.makeText(MedDescriptionAddActivity.this, "每天服务已关闭",Toast.LENGTH_SHORT).show();
				}else{
					System.out.println("repeat="+repeat);
					Toast.makeText(MedDescriptionAddActivity.this, "一次服务已关闭",Toast.LENGTH_SHORT).show();
				}
				//闹铃关
			}else{
				code = 1;
				...
				if(repeat == 0){
					System.out.println("repeat="+repeat);
					Toast.makeText(MedDescriptionAddActivity.this, "每天服务已启动",Toast.LENGTH_SHORT).show();
				}else{
					System.out.println("repeat="+repeat);
					Toast.makeText(MedDescriptionAddActivity.this, "一次服务已启动",Toast.LENGTH_SHORT).show();
				}
			}
		}
				...
				if (j == i) {
					...
					text.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							showDialog_Layout2(MedDescriptionAddActivity.this,text,view);
						}
		...
		//药品类型
		if (v == usageImage) {
			showDialog_Layout3(MedDescriptionAddActivity.this, usageText);
		}
		//重复间隔
		if (v == interval_layout) {
			showDialog_Layout4(MedDescriptionAddActivity.this, intervalText);
		}
		//铃声
		if (v == ringLayout) {
		...
		if (v == submit) {
			...
			medd.setTime(timeStr);*/
				...
				meddManager1.addMedDescription(medd);
			...
			public void onClick(View v) {
				...
				if (timeList.contains(text.getText().toString())) {
					AlertDialog.Builder builder = new AlertDialog.Builder(
							MedDescriptionAddActivity.this);
					builder.setTitle("注意");
					builder.setCancelable(false)
					.setMessage("不能添加相同时间")
					.setPositiveButton(
	...
	private long getTime(final TimePicker timePicker) {
		...
		if(systemTime > selectTime) {
			Toast.makeText(MedDescriptionAddActivity.this, "设置的时间小于当前时间", Toast.LENGTH_SHORT).show();
